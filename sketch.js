...
function preload() {
  sound = loadSound('sounds/1.wav'); // Default sound
}
...
function loadNewSound(url) {
  ...
  loadSound(url, function(newSound) {
    sound = newSound;
    sound.connect(fft);
    sound.play();
    startTime = millis();
...
function createMidiFile() {
  ...
  try {
    ...
    trackData = trackData.concat([
      ...
      0x07, 0xA1, 0x20        // 500000 in microseconds
    ]);

    // Set instrument (piano)
    trackData = trackData.concat([
      0x00, 0xC0, 0x00        // Program change to piano
    ]);

    // Sort notes by time
    const notes = midiNotes
      .sort((a, b) => a.time - b.time)
      .filter(note => note.midi >= 0 && note.midi <= 127 && note.duration > 0);
    ...
    notes.forEach((note, index) => {
      ...
      console.log(`Added note ${index}: ${note.note}, duration: ${duration} ticks`);
    });
...
function draw() {
  ...
  beginShape();
  for (let i = 0; i <= 180; i++) {
    let index = floor(map(constrain(i, 0, 180), 0, 180, 0, spectrum.length-1));
  ...
  beginShape();
  for (let i = 0; i <= 180; i++) {
    let index = floor(map(constrain(i, 0, 180), 0, 180, 0, soundWave.length-1));
  ...
  if (isRecording && sound && sound.isPlaying() && freq > 0 && musicalNote !== 'Unknown') {
    let now = millis();
...
function findPeaks(frequencies) {
  ...
  for (let octave = 0; octave < octaveBands; octave++) {
    let startBin = Math.floor(frequencies.length * Math.pow(2, octave-octaveBands));
    let endBin = Math.floor(frequencies.length * Math.pow(2, (octave-octaveBands+1)));
...
function mouseClicked() {
  ...
  if (!sound || mouseX < 0 || mouseX > width || mouseY < 0 || mouseY > height) return;
